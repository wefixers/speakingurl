...
describe('getSlug separator', () => {
  it('should separate with non-whitespace', () => {
    expect(getSlug('Foo Bar Baz', { separator: '-' })).toBe('foo-bar-baz')

    expect(getSlug('Foo Bar Baz', { separator: '*' })).toBe('foo*bar*baz')

    expect(getSlug('Foo Bar Baz', { separator: '_' })).toBe('foo_bar_baz')
  ...
  it('should separate with trailing separator "_"', () => {
    expect(getSlug('_Foo Bar Baz_', { separator: '_' })).toBe('foo_bar_baz')

    expect(getSlug('__Foo Bar Baz__', { separator: '_' })).toBe('foo_bar_baz')

    expect(getSlug('___Foo Bar Baz___', { separator: '_' })).toBe('foo_bar_baz')
  ...
  it('should remove trailing separator "*"', () => {
    expect(getSlug(' C\'est un beau titre qui ne laisse rien à désirer !', { separator: '*' })).toBe('c*est*un*beau*titre*qui*ne*laisse*rien*a*desirer')

    expect(getSlug(' C\'est un beau titre qui ne laisse rien à désirer !', '*')).toBe('c*est*un*beau*titre*qui*ne*laisse*rien*a*desirer')
  })

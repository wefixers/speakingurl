...
describe('getSlug symbols', () => {
  it('should convert symbols', () => {
    ...
    expect(getSlug('Foo & Bar | Baz', { lang: 'ro' })).toBe('foo-si-bar-sau-baz')

    expect(getSlug('Foo & Bar | Baz', { lang: 'sk' })).toBe('foo-a-bar-alebo-baz')
  })

  it('shouldn\'t convert symbols', () => {
    expect(getSlug('Foo & Bar | Baz', { symbols: false })).toBe('foo-bar-baz')

    expect(getSlug('Foo & Bar | Baz', { lang: 'en', symbols: false })).toBe('foo-bar-baz')

    expect(getSlug('Foo & Bar | Baz', { lang: 'de', symbols: false })).toBe('foo-bar-baz')

    expect(getSlug('Foo & Bar | Baz', { lang: 'fr', symbols: false })).toBe('foo-bar-baz')

    expect(getSlug('Foo & Bar | Baz', { lang: 'es', symbols: false })).toBe('foo-bar-baz')

    expect(getSlug('Foo & Bar | Baz', { lang: 'ru', symbols: false })).toBe('foo-bar-baz')

    expect(getSlug('Foo & Bar | Baz', { lang: 'cs', symbols: false })).toBe('foo-bar-baz')

    expect(getSlug('Foo & Bar | Baz', { lang: 'sk', symbols: false })).toBe('foo-bar-baz')
  })
  ...
  it('should not convert symbols with mark flag true', () => {
    expect(getSlug('Foo (Bar) . Baz', { mark: true })).toBe('foo-(bar)-.-baz')
  ...
  it('should replace chars by se language standards', () => {
    expect(getSlug('AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZzÅåÄäÖö', { lang: 'sv', maintainCase: true })).toBe('AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZzAaAaOo')
  })

  it('should replace chars by fi language standards', () => {
    expect(getSlug('AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZzÅåÄäÖö', { lang: 'fi', maintainCase: true })).toBe('AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZzAaAaOo')
  })

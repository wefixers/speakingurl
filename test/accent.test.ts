...
describe('getSlug translate sentence with accent words', () => {
  ...
  describe('titlecase options', () => {
    const getSlug = createSlug({
      titleCase: ['a', 'an', 'and', 'as', 'at', 'but', 'by', 'en', 'for', 'if', 'in', 'nor', 'of', 'on', 'or', 'per', 'the', 'to', 'vs'],
    })
    ...
    it('beginning of sentence', () => {
      expect(getSlug('Ánhanguera foo Ánhanguera')).toBe('Anhanguera-Foo-Anhanguera')
    })

    it('beginning of sentence, with exception', () => {
      expect(getSlug('Ánhanguera and Ánhanguera')).toBe('Anhanguera-and-Anhanguera')
    })
